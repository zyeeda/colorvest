  ...
  define(['jquery', 'underscore', 'coala/core/view', 'coala/core/config', 'coala/scaffold/abstract-view-loader'], function($, _, View, config, viewLoader) {
    ...
    handlers = {
      add: function() {
        ...
        grid = this.feature.views['treeTableViews:grid'].components[0];
        selected = grid.getGridParam('selrow');
        if (selected) {
          rowData = grid.getRowData(selected);
          rowData.id = selected;
          this.feature.views['forms:add'].model.set('parent', rowData);
        }
          ...
          viewLoader.generateGridView({
            ...
            handlers: {
              selectionChanged: function(id, status) {
                ...
                v = this.feature.views['treeTableViews:operators'];
