        ...
        populate = function (npage) {
            if(!ts.grid.hDiv.loading) {
                ...
                case "collection":
                    ...
                    var op = {data: ts.p.postData};
                    if (op.data.page) {
                        op.data["_page"] = op.data.page;
                    }
                    if (op.data.rows) {
                        op.data["_pageSize"] = op.data.rows;
                    }
                    if (op.data.sidx) {
                        op.data["_order"] = op.data.sidx + "-" + op.data.sord;
                    }
                    if (op.data.filters) {
                    ...
                    delete op.data.sidx;
                    delete op.data.sord;
                    delete op.data.filters;

                    collection = ts.p.collection;
                    collection.fetch(op).done(function () {
                        data = {
                ...
                case "tree-table-collection":
                    ...
                    collection.fetch(op).done(function () {
                        ...
                        rs = [];
                        processRecords(data.rows, null, 0, rs);
                        data.rows = rs;
    ...
    setGridWidth : function(nwidth, shrink) {
        return this.each(function(){
            ...
            if(shrink===true) {
                ...
                aw = nwidth-brd*vc-gw;
                if(!isNaN($t.p.height)) {
                    if ($t.p.alwaysShowVerticalScroll
                       || ($($t.grid.bDiv)[0].clientHeight < $($t.grid.bDiv)[0].scrollHeight)
                       || $t.rows.length === 1) {
                        hs = true;
                        aw -= scw;
                    }
                    /*if(k($($t.grid.bDiv)[0].clientHeight < $($t.grid.bDiv)[0].scrollHeight) || $t.rows.length === 1){
                        hs = true;
                        aw -= scw;
                    }*/
                }
